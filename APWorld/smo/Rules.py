...
def set_rules(self, options : SMOOptions) -> None:
    ...
    self.multiworld.get_location("Big Broodal Battle" , self.player).place_locked_item(self.create_item("Bowser's Power Moon (314)"))
    ...
    set_rule(self.multiworld.get_location("A Traditional Festival!", self.player), lambda state: state.has("Metro Story Moon (5)", self.player))

    # Sand Story Progress
    set_rule(self.multiworld.get_location("Moon Shards in the Sand", self.player), lambda state: state.has("Sand Story Moon (1)"))
